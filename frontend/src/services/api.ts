...
class ApiService {
  ...
  async login(credentials: LoginCredentials): Promise<AuthResponse> {
    try {
      const response: AxiosResponse<AuthResponse> = await this.api.post('/accounts/login/', credentials);
      if (response.data.sucesso) {
        localStorage.setItem('access_token', response.data.dados.access);
        localStorage.setItem('refresh_token', response.data.dados.refresh);
        localStorage.setItem('user', JSON.stringify(response.data.dados.user));
      }
